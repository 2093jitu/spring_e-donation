...
public class LoninOrRegeDaoImpl implements LoninOrRegeDao {
	...
	public DonationRege getById(String name) {		
		...
		return (DonationRege) sessionFactory.getCurrentSession().createQuery("From DonationRege d where d.email='"+name+"'").uniqueResult();
	...
	public DonationRege getById(int id) {	
		...
		return (DonationRege) sessionFactory.getCurrentSession().createQuery("From DonationRege d where d.id='"+id+"'").uniqueResult();
	}
